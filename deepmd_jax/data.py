...
class DPDataset():
    ...
    def _get_stats(self, rcut, bs, iterw=False):
        if self.is_leaf:
            ...
            sr_BnM = [sr(jnp.concatenate(r,axis=-1), rcut, z=2 if iterw else 0) for r in r_Bnm]
            ...
            s = np.stack([subset._get_stats(rcut, bs, iterw) for subset in self.subsets], axis=-1)
            return (s * self.prob).sum(-1)
        
    def get_stats(self, rcut, bs, iterw=False):
        self.params = {'ntypes': self.ntypes, 'rcut': rcut}
        sr_sum, sr_sum2, sr_count, Nnbrs = self._get_stats(rcut, bs, iterw)
        ...
        if iterw:
            self.params['valid_types'] = np.concatenate([self.params['valid_types'], [self.params['ntypes']]])
            self.params['ntypes'] += 1
            self.params['sr_mean'] = np.concatenate([self.params['sr_mean'], [np.max(self.params['sr_mean'])]])
            self.params['sr_std'] = np.concatenate([self.params['sr_std'], [np.max(self.params['sr_std'])]])
        return self.params
